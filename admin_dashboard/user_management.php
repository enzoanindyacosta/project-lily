...
<head>
  ...
  <title>User Management | Project Lilly | AIDA</title>
...
<body>
  ...
  <div class="container p-3 my-3 shadow-sm bg-body rounded-2 border border-1 border-bg">
    <div class="d-flex">
      ...
      <a class="btn btn-primary fw-bold text-uppercase px-4 w-100" href="user_management.php">User Management</a>
      <span class="px-2"></span>
  ...
  <div class="container p-3 my-3 shadow-sm bg-body rounded-2 border border-1 border-bg">
    <div class="d-flex justify-content-between align-items-center mb-2 border-bottom border-1 border-secondary pb-3">
      <span>
        <h4 class="text-primary m-0 p-0">User List</h4>
        <?php
      ...
      <span>
        ...
        if (isset($_GET['error']) || isset($_GET['success'])) {
          echo '<button class="btn btn-outline-danger btn-sm me-2 px-3" onclick="window.location.href=\'user_management.php\'">Clear Notification</button>';
        }
        ?>
        <a class="btn btn-success btn-sm px-3" href="add_user.php">Add New User</a>
      </span>
    </div>
    <span class="text-danger mb-3 d-block">
      *Note: Deleting a user will also delete all the stories created by the user.
    </span>
    <table id="user_list" class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination pt-2">
      <thead>
        <tr>
          <th>No.</th>
          <th>Fullname</th>
        ...
        ?>
            <tr>
              ...
              <td style="width: 10rem; text-align: center;">
                <a class="btn btn-outline-primary btn-sm px-3" href="edit_user.php?id=<?php echo $id; ?>">Edit</a>
                <a class="btn btn-outline-danger btn-sm px-3" href="delete_user.php?id=<?php echo $id; ?>" onclick="return confirm('Are you sure you want to delete this user?')">Delete</a>
              </td>
            </tr>
        <?php
          }
