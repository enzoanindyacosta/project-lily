...
<head>
  ...
  <title>Story Category | Project Lilly | AIDA</title>
...
<body>
  ...
  <div class="container p-3 my-3 shadow-sm bg-body rounded-2 border border-1 border-bg">
    <div class="d-flex justify-content-between align-items-center mb-2 border-bottom border-1 border-secondary pb-3">
      <span>
        <h4 class="text-primary m-0 p-0">Category List</h4>
        <?php
      ...
      <span>
        ...
        if (isset($_GET['error']) || isset($_GET['success'])) {
          echo '<button class="btn btn-outline-danger btn-sm me-2 px-3" onclick="window.location.href=\'story_category.php\'">Clear Notification</button>';
        }
        ?>
        <a class="btn btn-success btn-sm px-3" href="add_category.php">Add New Category</a>
      </span>
    </div>
    <span class="text-danger mb-3 d-block">
      *Note: Deleting a category will also delete all the stories under that category.
    </span>
    <table id="category_list" class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination pt-2">
      <thead>
        <tr>
          <th>No.</th>
          <th>Title</th>
        ...
        ?>
            <tr>
              ...
              <td style="width: 10rem; text-align: center;">
                <a class="btn btn-outline-primary btn-sm px-3" href="edit_category.php?id=<?php echo $id; ?>">Edit</a>
                <a class="btn btn-outline-danger btn-sm px-3" href="delete_category.php?id=<?php echo $id; ?>" onclick="return confirm('Are you sure you want to delete this category?')">Delete</a>
              </td>
            </tr>
        <?php
          }
